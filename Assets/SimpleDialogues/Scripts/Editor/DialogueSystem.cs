...
public class DialogueSystem : EditorWindow {
    ...
    Dialogues.Window CreateNewWindow(Vector2 Position, int ParentId, 
									Dialogues.WindowTypes Type = Dialogues.WindowTypes.Text)
    {
        Dialogues.Window NewWindow = new Dialogues.Window(CurrentDialogue.Set[CurrentDialogue.CurrentSet].CurrentId, 
									ParentId, new Rect(Position, WindowSize), Type);
    ...
    {
		Dialogues.Window currentDialogWindow = CurrentDialogue.Set [CurrentDialogue.CurrentSet].FirstWindow;

        if (currentDialogWindow == null || currentDialogWindow.Connections == null)
            return false;
        else
            return true;
    }
    ...
    void CreateConnection(object win)
    {
        ...
        {
            if (!ConnectingCurrent.Connections.Contains(Curr.ID))// && ConnectingCurrent.Type == WindowTypes.Decision)
            {
        ...
        for (int j = 0; j < CurrentDialogue.Set[CurrentDialogue.CurrentSet].Windows.Count; j++)
        {
            ...
            if (PrevWindow != null)
            {
                for (int i = 0; i < PrevWindow.Count; i++)
                {
                    ...
                    if (PrevWindow[i].Type == Dialogues.WindowTypes.Text && WindowList.Type == Dialogues.WindowTypes.Option)
                        WindowList.Type = Dialogues.WindowTypes.Text;
                }
            }
            ...
            switch (WindowList.Type)
            {
                case Dialogues.WindowTypes.Text:
                    BoxName = "Dialogue";
                    break;
                case Dialogues.WindowTypes.Decision:
                    BoxName = "Decision";
                    break;
                case Dialogues.WindowTypes.Option:
                    BoxName = "Option";
                    break;
            }
            //Determines what type of node it is
            if (WindowList.Connections.Count == 0 && WindowList != CurrentDialogue.Set[CurrentDialogue.CurrentSet].FirstWindow && WindowList.Type != Dialogues.WindowTypes.Decision)
            ...
            if (WindowList.Type == Dialogues.WindowTypes.Option) Style = "flow node 2";
            if (WindowList.NodeType == Dialogues.NodeType.End) Style = "flow node 6";
            if (WindowList.Type == Dialogues.WindowTypes.Decision && WindowList.Connections.Count == 0) Style = "flow node 4";
    ...
    void BuildMenus(GenericMenu Menu)
    {
        ...
        for (int j = 0; j < CurrentDialogue.Set[CurrentDialogue.CurrentSet].Windows.Count; j++)
        {
            ...
            if (Connecting)
            {
                ...
                if (ConnectingCurrent.Type == Dialogues.WindowTypes.Text && ConnectingCurrent.Connections.Count == 0 || ConnectingCurrent.Type == Dialogues.WindowTypes.Option && ConnectingCurrent.Connections.Count == 0 || ConnectingCurrent.Type == Dialogues.WindowTypes.Decision)
                {
            ...
            if (AdjustedArea.Contains(Event.current.mousePosition))
            {
                ...
                else
                {
                    ...
                    if (WindowList.Type == Dialogues.WindowTypes.Text && WindowList.Connections.Count == 0 || WindowList.Type == Dialogues.WindowTypes.Option && WindowList.Connections.Count == 0 || WindowList.Type == Dialogues.WindowTypes.Decision)
    ...
    private void OnGUI()
    {
		// Renders things on the editor window
        ...
        else
        {
            ...
            ScrollPosition = GUI.BeginScrollView(	new Rect(0, 0, position.width, position.height), 
													ScrollPosition, 
													new Rect(Vector2.zero, WorkSize), 
													GUIStyle.none, 
													GUIStyle.none);
